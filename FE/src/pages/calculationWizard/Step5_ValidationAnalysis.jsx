...
const Step5ValidationAnalysis = ({ onBack, onComplete }) => {
    ...
    useEffect(() => {
        ...
        if (draftData.exportFormat) {
            setExportFormat(draftData.exportFormat);
        }
    }, []);
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            draftData.exportFormat = exportFormat;
            localStorage.setItem('gearbox_draft', JSON.stringify(draftData));
            alert('✅ Đồ án đã được lưu thành công!');
        } catch (error) {
            alert('❌ Lỗi khi lưu đồ án: ' + error.message);
        } finally {
            setIsSaving(false);
        }
    };

    const handleApplyOptimization = (type) => {
        alert(`✨ Đang áp dụng tối ưu hóa: ${type}\n\nHệ thống sẽ cập nhật các thông số thiết kế...`);
        // This will trigger recalculation in Step 3
        const draftData = JSON.parse(localStorage.getItem('gearbox_draft') || '{}');
        if (type === 'material') {
            draftData.gearMaterial = '18CrNiMo7-6';
        } else if (type === 'profile') {
            draftData.helixAngle = 22.5;
        }
        localStorage.setItem('gearbox_draft', JSON.stringify(draftData));
    };
    ...
    const handleExportReport = async () => {
        ...
        try {
            ...
            const reportContent = `
BÁNG TÁI THIẾT KẾ HỘP GIẢM TỐC KIẠN
==========================================
...
TÓNG TẮT LẬP:
- Công suất: ${draftData.power || 'N/A'} kW
- Tốc độ: ${draftData.speed || 'N/A'} v/p
- Tỉ truyền: ${draftData.transmissionRatio || 'N/A'}

KÊÛT QUẢ KIỂM TRA ỨNG SUẤT:
- Contact Stress: SAFE (842 MPa)
- Root Bending: SAFE (215 MPa)
- Safety Factor: 1.12 SF (LƯU Ý: Dưới 1.25 cho ứng dụng)

XUẤ BÁNG TÁI THÀNH CÔNG!
            `;
            ...
            link.href = url;
            link.download = `Thugai_Giảm_Tốc_${new Date().getTime()}.${exportFormat === 'pdf' ? 'pdf' : exportFormat}`;
            document.body.appendChild(link);
            link.click();
            link.remove();
            
            alert(`✅ Báng tái được xuất thành công (${exportFormat.toUpperCase()})!`);
            onComplete();
        } catch (error) {
            alert('❌ Lỗi khi xuất báng tái: ' + error.message);
        } finally {
                ...
                <div className="max-w-6xl mx-auto grid grid-cols-12 gap-6">
                    <div className="col-span-12 lg:col-span-4 space-y-6">
                        ...
                        <section className="bg-slate-900 rounded-xl p-6 text-white relative shadow-xl">
                            ...
                            <div className="space-y-3">
                                <button onClick={() => handleApplyOptimization('material')} className="w-full text-left p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-bold text-blue-300 uppercase">Material Upgrade</span>
                                        <span className="material-symbols-outlined text-xs">arrow_forward</span>
                                    </div>
                                    <p className="text-[11px] text-slate-400 mt-1">Switch to 18CrNiMo7-6 for +15% fatigue strength.</p>
                                </button>
                                <button onClick={() => handleApplyOptimization('profile')} className="w-full text-left p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-bold text-blue-300 uppercase">Profile Adjustment</span>
                                        <span className="material-symbols-outlined text-xs">arrow_forward</span>
                                    </div>
                                    <p className="text-[11px] text-slate-400 mt-1">Increase Helix Angle to 22.5° to distribute load.</p>
                                </button>
                            </div>
                        </section>
                    </div>
                    ...
                    <div className="col-span-12 lg:col-span-8 space-y-6">
                        ...
                        <div className="flex justify-between items-center pt-4">
                            <button onClick={onBack} className="flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-200">
                            ...
                            <div className="flex gap-4">
                                <button 
                                    ...
                                    disabled={isSaving}
                                    className="flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold border border-[#c2c6d6] text-slate-700 hover:bg-white disabled:opacity-50">
                                ...
                                <select 
                                    ...
                                    value={exportFormat} 
                                    onChange={(event) => handleChangeFormat(event.target.value)} 
                                    className="px-3 py-2.5 rounded-lg text-sm border border-[#c2c6d6] bg-white">
                                ...
                                <button 
                                    ...
                                    className="gradient-button flex items-center gap-2 px-8 py-2.5 rounded-lg text-sm font-bold text-white shadow-lg hover:shadow-blue-500/20 disabled:opacity-50">
                                    <span className="material-symbols-outlined">{isExporting ? 'hourglass_empty' : 'description'}</span>
                                    {isExporting ? 'Đang xuất...' : 'Export Full Report'}
                                </button>
                            </div>
                        </div>
                    </div>
